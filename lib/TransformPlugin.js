...
class TransformPlugin {
  ...
  addHooks() {
    ...
    this.cli.on('ti.transform.post', {
      priority: 10000,
      post: this.handlePostTransform
    });
  ...
  handlePostTransform(data, next) {
    this.writeLock(data.lock || {})
      .then(() => next())
      .catch(e => next(e));
  }
  ...
  transform() {
    return this.readLock()
      .then((lock = {}) => {
        ...
        return this.emit('ti.transform.pre')
          .then(() => Promise.all(changedSrcFiles.map(({ path: src, stats }) => {
            const data = {
              src,
              gen: [],
              extra: {},
      ...
      .then(({ newLock }) => this.emit('ti.transform.post', { lock: newLock }));
  }
  ...
  emit(hookName, context) {
    return new Promise((resolve, reject) => {
      this.cli.emit(hookName, Object.assign(context || {}, { paths: this.paths }), (err, result) =>
